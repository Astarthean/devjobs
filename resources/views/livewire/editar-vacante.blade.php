<form class="md:w-1/2 space-y-4" wire:submit.prevent='editarVacante'>
    ...
    <div>
        ...
        <div class="my-5 w-80">
            <x-input-label :value="__('Imagen Actual')" class="text-sm text-green-600 dark:text-green-400 mb-2" />
            <img src="{{ asset('storage/public/vacantes/' . $imagen) }}" alt="{{ 'Imagen Vacante' . $imagen }}">
        </div>

        <div class="my-5">
            @if ($imagen_nueva)
                <p class="text-sm text-green-600 dark:text-green-400">{{ __('Imagen subida') }}</p>
                <img src="{{ $imagen_nueva->temporaryUrl() }}" alt="Imagen de la vacante" class="w-80 object-cover mt-2">
            @endif
        </div>
